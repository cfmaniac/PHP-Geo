...
foreach($xml->result->address_component as $component){
	...
	if($component->type=='administrative_area_level_1'){
		$geodata['region_state'] = $component->short_name;
	}
...
$geodata['state'] = $xml->result->administrative_area_level_1->short_name;
...
foreach($geodata as $name => $value){
	if($name == "town_city") {
		...
		echo $value .', '. $geodata['region_state'];
	}
}
//Displays the GeoLocation information:
echo '<p><img src="http://maps.google.com/maps/api/staticmap?center='.$lat.','.$long.'&zoom=14&size=150x150&maptype=roadmap&&sensor=true" width="300" height="300" alt="'.$geodata['formatted_address'].'" \/></p>';
echo 'Latitude: '.$lat.' Longitude: '.$long.'<br />';
foreach($geodata as $name => $value){
	echo '<p><strong>'.$name.'</strong>: '.str_replace('&','&amp;',$value).'</p>';
}
